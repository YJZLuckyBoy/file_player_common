...
#ifdef COMPILE_LIVOX
    map<int64_t, livox_ros_driver::CustomMsg>         ouster_data_;
#else
    map<int64_t, sensor_msgs::PointCloud2>         ouster_data_;
#endif
    // map<int64_t, irp_sen_msgs::imu>         imu_data_origin_;
    map<int64_t, sensor_msgs::MagneticField>         mag_data_;
    map<int64_t, cv::Mat>                     camera_data_;
...
#ifdef COMPILE_LIVOX
    pair<string,livox_ros_driver::CustomMsg> ouster_next_;
#else
