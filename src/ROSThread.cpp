...
ROSThread::ros_initialize(ros::NodeHandle &n)
{
  ...
  nh_.param<std::string>("file_dir", file_dir_, " ");
  cv::FileStorage fs(file_dir_, cv::FileStorage::READ);

  if (!fs.isOpened())
      std::cout << "Can not open " + file_dir_ << std::endl;

  cv::FileNode ns = fs["topic_name"];
  std::string lidar_topic, imu_topic, gps_topic, camera_topic;
  ns["lidar_topic"]  >> lidar_topic;   topics_.push_back(lidar_topic);
  ns["imu_topic"]    >> imu_topic;     topics_.push_back(imu_topic);
  ns["gps_topic"]    >> gps_topic;     topics_.push_back(gps_topic);
  ns["camera_topic"] >> camera_topic;  topics_.push_back(camera_topic);
...
#ifdef COMPILE_LIVOX
  ouster_pub_ = nh_.advertise<livox_ros_driver::CustomMsg>("/livox/lidar", 1000); // giseop
#else
  ouster_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/points_raw", 1000); // giseop
#endif
  radarpolar_pub_ = nh_.advertise<sensor_msgs::Image>("/camera/image_color", 10); // giseop
}
...
ROSThread::Ready()
{
  ...
  rosbag::Bag bag;
  bag.open(data_folder_path_, rosbag::bagmode::Read);
  ...
  foreach (rosbag::MessageInstance const m, view)
  {
#ifdef COMPILE_LIVOX
      // add lidar msg
      livox_ros_driver::CustomMsgConstPtr lidar_livox_msg = m.instantiate<livox_ros_driver::CustomMsg>();
      if (lidar_livox_msg != NULL)
      {
        int64_t stamp = lidar_livox_msg->header.stamp.toNSec();
        data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "velodyne"));
        ouster_file_list_.push_back(std::to_string(stamp));
        ouster_data_[stamp] = *lidar_livox_msg;
      }
#else
      // add lidar msg
      ...
      if (lidar_msg != NULL)
      {
        int64_t stamp = lidar_msg->header.stamp.toNSec();
        data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "velodyne"));
        ouster_file_list_.push_back(std::to_string(stamp));
        ouster_data_[stamp] = *lidar_msg;
      }
#endif
      ...
      if (imu_msg)
      {
          int64_t stamp = imu_msg->header.stamp.toNSec();
          data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "imu"));
          imu_data_[stamp] = *imu_msg;
      }

      // add gps msg
      sensor_msgs::NavSatFixConstPtr gps_msg = m.instantiate<sensor_msgs::NavSatFix>();
      if (gps_msg)
      {
          int64_t stamp = gps_msg->header.stamp.toNSec();
          data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "gps"));
          gps_data_[stamp] = *gps_msg;
      }

      // add camera msg
      sensor_msgs::ImageConstPtr camera_msg = m.instantiate<sensor_msgs::Image>();
      if (camera_msg != NULL)
      {
        cv_bridge::CvImageConstPtr ptr;
        ptr = cv_bridge::toCvCopy(camera_msg, sensor_msgs::image_encodings::BGR8);
        int64_t stamp = camera_msg->header.stamp.toNSec();
        data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "camera"));
        radarpolar_file_list_.push_back(std::to_string(stamp));
        camera_data_[stamp] = ptr->image;
      }

      // add camera msg
      sensor_msgs::CompressedImageConstPtr camera_compressed_msg = m.instantiate<sensor_msgs::CompressedImage>();
      if (camera_compressed_msg != NULL)
      {
        cv_bridge::CvImageConstPtr ptr;
        ptr = cv_bridge::toCvCopy(camera_compressed_msg, sensor_msgs::image_encodings::BGR8);
        int64_t stamp = camera_compressed_msg->header.stamp.toNSec();
        data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, "camera"));
        radarpolar_file_list_.push_back(std::to_string(stamp));
        camera_data_[stamp] = ptr->image;
      }
  ...
  std::cout << "lidar  data size: "  << ouster_data_.size() << std::endl;
  std::cout << "imu    data size: "  << imu_data_.size()    << std::endl;
  std::cout << "gps    data size: "  << gps_data_.size()    << std::endl;
  std::cout << "camera data size: "  << camera_data_.size() << std::endl;

  if (ouster_data_.empty() && imu_data_.empty() && gps_data_.empty() && camera_data_.empty())
    ROS_ERROR("All sensor data is empty, please check your 'topic.yaml' file, maybe all topics is incorrect!");
...
ROSThread::DataStampThread()
{
  ...
  for(auto iter = data_stamp_.begin() ; iter != data_stamp_.end() ; iter ++)
  {
    ...
    else if(iter->second.compare("camera") == 0 && radarpolar_active_ == true)
    {
...
ROSThread::OusterThread()
{
  ...
  while(1)
  {
    ...
    while(!ouster_thread_.data_queue_.empty())
    {
      ...
      if (std::to_string(data) == ouster_next_.first)
      {
        //publish
        ouster_next_.second = ouster_data_[data];
        ouster_next_.second.header.stamp.fromNSec(data);
        ouster_next_.second.header.frame_id = "velodyne"; // frame ID
      ...
      else
      {
        ...
        if(find(next(ouster_file_list_.begin(),max(0,previous_file_index-search_bound_)),ouster_file_list_.end(),to_string(data)) != ouster_file_list_.end())
        {
          // ifstream file;
          ...
          // pcl::toROSMsg(ouster_data_[data], publish_cloud);
          // publish_cloud.header.stamp.fromNSec(data);
          // publish_cloud.header.frame_id = "ouster";

          ouster_data_[data].header.stamp.fromNSec(data);
          ouster_data_[data].header.frame_id = "velodyne";

          ouster_pub_.publish(ouster_data_[data]);
        }
      ...
      // cloud.clear();
      // sensor_msgs::PointCloud2 publish_cloud;
      current_file_index = find(next(ouster_file_list_.begin(),max(0,previous_file_index-search_bound_)),ouster_file_list_.end(),to_string(data)) - ouster_file_list_.begin();
      if(find(next(ouster_file_list_.begin(),max(0,previous_file_index-search_bound_)),ouster_file_list_.end(),ouster_file_list_[current_file_index+1]) != ouster_file_list_.end()){
        ...
        // file.close();
        // pcl::toROSMsg(ouster_data_[current_file_index+1], publish_cloud);
        ouster_next_ = make_pair(ouster_file_list_[current_file_index+1], ouster_data_[current_file_index+1]);
      }
...
ROSThread::RadarpolarThread()
{
  ...
  while(1){
    ...
    while(!radarpolar_thread_.data_queue_.empty())
    {
      ...
      if( to_string(data) == radarpolar_next_.first)
      {
        cv_bridge::CvImage radarpolar_out_msg;
        radarpolar_out_msg.header.stamp.fromNSec(data);
        radarpolar_out_msg.header.frame_id = "camera";
        radarpolar_out_msg.encoding = sensor_msgs::image_encodings::BGR8;
        radarpolar_out_msg.image    = camera_data_[data];
      ...
      else
      {
        ...
        cv::Mat radarpolar_image = camera_data_[data];
      ...
      if(current_img_index < radarpolar_file_list_.size()-2)
      {
        ...
        radarpolar_next_ = make_pair(radarpolar_file_list_[current_img_index+1], camera_data_[current_img_index+1]);
      }
